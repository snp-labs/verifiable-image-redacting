...
namespace libsnark
{
	...
	public:
		...
		snark_for_filtering_Commit(const libff::G1<ppT> &&sigma_x,
								   const libff::Fr<ppT> &&x0) : sigma_x(std::move(sigma_x)),
																x0(std::move(x0)){};
	};
	template <typename ppT>
	...
	public:
		...
		snark_for_filtering_public_parameter(
			const libff::G1_vector<ppT> &&h_vector) : h_vector(std::move(h_vector)){};
 ...
 */
	...
	public:
		...
		snark_for_filtering_keypair(const snark_for_filtering_keypair<ppT> &other) : pk(std::move(other.pk)),
																					 vk(std::move(other.vk)),
																					 pp(std::move(other.pp)){};
		snark_for_filtering_keypair(snark_for_filtering_proving_key<ppT> &&pk,
									snark_for_filtering_verification_key<ppT> &&vk,
									snark_for_filtering_public_parameter<ppT> &&pp) : pk(std::move(pk)),
																					  vk(std::move(vk)),
																					  pp(std::move(pp)){};
	...
	public:
		...
		size_t size_in_bits() const
		{
			return proof_size() * libff::G1<ppT>::size_in_bits() + libff::G2<ppT>::size_in_bits();
		}
		void print_size() const
		{
			libff::print_indent();
			printf("* snark_for_filtering_proof size in bits: %zu\n", this->size_in_bits());
		}
