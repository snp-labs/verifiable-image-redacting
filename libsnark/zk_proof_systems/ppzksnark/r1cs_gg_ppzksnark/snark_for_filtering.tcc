...
template <typename ppT>
snark_for_filtering_keypair<ppT> snark_for_filtering_generator(const r1cs_gg_ppzksnark_constraint_system<ppT> &r1cs){
// void snark_for_filtering_generator(const r1cs_constraint_system<libff::Fr<ppT> > &r1cs){
    ...
    r1cs_gg_ppzksnark_keypair<ppT> keypair = r1cs_gg_ppzksnark_generator<ppT>(r1cs);
    // libff::G1_vector<ppT> f_vector = keypair.pk.L_query;

    //Use h_vector, f_vector to build a matrix M
    // P <- M^T * k
    P_vector.emplace_back(k2 * keypair.pk.L_query[0]);
    P_vector.emplace_back(k1 * keypair.pk.L_query[num_variables/2]);
    P_vector.emplace_back(k0 * h_vector[0]);

    for(size_t i = 1; i < num_variables/2; i++){
		P_vector.emplace_back((k0 * h_vector[i]) + (k2 * keypair.pk.L_query[i]));
    }
    for(size_t i = 1; i < num_variables/2; i++){
		P_vector.emplace_back((k0 * h_vector[i]) + (k1 * keypair.pk.L_query[num_variables/2+i]));
    }

    snark_for_filtering_proving_key<ppT> ek = snark_for_filtering_proving_key<ppT>(
        std::move(P_vector),
        std::move(keypair.pk.L_query),
...
snark_for_filtering_proof<ppT> snark_for_filtering_prover(snark_for_filtering_proving_key<ppT> &pk, 
    ...
    r1cs_gg_ppzksnark_proving_key<ppT> completment_pk = r1cs_gg_ppzksnark_proving_key<ppT>(
    ...
    r1cs_gg_ppzksnark_proof<ppT> completment_proof = r1cs_gg_ppzksnark_prover<ppT>(
...
bool snark_for_filtering_verifier(const snark_for_filtering_verification_key<ppT> &vk, 
    ...
    r1cs_gg_ppzksnark_verification_key<ppT> completment_vk = r1cs_gg_ppzksnark_verification_key<ppT>(
        std::move(vk.alpha_g1_beta_g2),
        std::move(vk.gamma_g2),
        std::move(vk.delta_g2),
        std::move(vk.gamma_ABC_g1)
        );
    ...
    return (left == (right0 *  right1 * right2) && 
    // return r1cs_gg_ppzksnark_verifier_weak_IC(completment_vk, proof._C_x, C_x, proof.completment_proof);
            r1cs_gg_ppzksnark_verifier_weak_IC<ppT>(completment_vk, proof._C_x, C_x, proof.completment_proof));
}
}// libsnark
